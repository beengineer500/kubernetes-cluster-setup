...
#==============================================================================
# Kubernetes GPU Worker Node Setup Script
# Description: 쿠버네티스 GPU 워커 노드 환경을 자동으로 구성하는 스크립트
# Version: 1.0
#==============================================================================
...
step3_hostname_hosts() {
    ...
    # 호스트명 설정 (워커 노드)
    read -p "워커 노드 호스트명을 입력하세요 [k8s-gpu-worker01]: " hostname
    hostname=${hostname:-k8s-gpu-worker01}
    ...
    grep -q "k8s-worker01" /etc/hosts || echo "192.168.35.80 k8s-gpu-worker01" >> /etc/hosts
    
    info "hosts 파일 내용:"
    cat /etc/hosts | grep -E "(k8s-master|k8s-gpu-worker)"

...
# Description: 쿠버네티스 멀티 마스터 클러스터의 마스터 노드 환경을 자동으로 구성하는 스크립트
# Author: Beengineer
# Version: 1.0
#==============================================================================
...
step3_hostname_hosts() {
    ...
    # 호스트명 설정 (워커 노드)
    read -p "노드 호스트명을 입력하세요 [k8s-multi-master0x]: " hostname
    hostname=${hostname:-k8s-multi-master01}
    
    hostnamectl set-hostname "${hostname}" || { error "호스트명 설정 실패"; return 1; }
    
    # hosts 파일 설정
    # 기존 항목 확인 후 추가
    grep -q "k8s-multi-master01" /etc/hosts || echo "192.168.35.71 k8s-multi-master01" >> /etc/hosts
    grep -q "k8s-multi-master02" /etc/hosts || echo "192.168.35.72 k8s-multi-master02" >> /etc/hosts
    grep -q "k8s-multi-master03" /etc/hosts || echo "192.168.35.73 k8s-multi-master03" >> /etc/hosts
    
    info "hosts 파일 내용:"
    cat /etc/hosts | grep -E "(k8s-multi-master|k8s-worker)"
